import React from "react";
import '../Css/Main.css';
import {Dropdown} from 'react-bootstrap';

function Main(){
    return(        
        <div className="main-master">
            <div className="main-wrap">
                <div className="main-header">
                    <div className="main-header-util">
                        <ul className="main-header-ul">
                            <li className="main-header-ul-li">
                                <div className="main-header-ul-li-trend">
                                    <div className="main-header-ul-li-trend-image">
                                    </div>
                                    <p className="main-header-ul-li-trend-p">
                                        트렌딩
                                    </p>
                                </div>
                            </li>
                            <li className="main-header-ul-li">
                                <div className="main-header-ul-li-latest">
                                    <div className="main-header-ul-li-latest-image">

                                    </div>
                                    <p className="main-header-ul-li-latest-p">
                                        최신
                                    </p>
                                </div>
                            </li>
                        </ul>
                        
                        <div className="main-header-util-span">                            
                            <p className="main-header-util-span-p">이번 주</p>
                            <div className="main-header-util-span-image"></div>
                        </div>
                        <div className="main-header-until-span-dropbox">   
                            <ul className="main-header-until-span-dropbox-ul">
                                <li className="main-header-until-span-dropbox-ul-li">오늘</li>
                                <li className="main-header-until-span-dropbox-ul-li">이번 주</li>
                                <li className="main-header-until-span-dropbox-ul-li">이번 달</li>
                                <li className="main-header-until-span-dropbox-ul-li">올해</li>
                            </ul>     
                        </div>
                    </div>
                    <div className="main-header-option">
                        <div className="main-header-option-dot">                            
                        </div>
                    </div>
                </div>
                <div className="main-card">
                    <div className="main-card-block sparkling">
                        <div className="main-card-block-image"></div>
                        <div className="main-card-block-main">
                            <div className="main-card-block-main-title">
                                Hello Guys!!!
                            </div>
                            <div className="main-card-block-main-content">
                                Hoodie is so Cooool!
                            </div>
                            <div className="main-card-block-main-date">
                                2022-03-17
                            </div>
                        </div>
                        <div className="main-card-block-tag ">
                            <div className="main-card-block-tag-id">
                                thewooowon
                            </div>
                            <div className="main-card-block-tag-like">
                                64
                            </div>
                        </div>
                    </div>
                    <div className="main-card-block sparkling">
                        <div className="main-card-block-image"></div>
                        <div className="main-card-block-main">
                            <div className="main-card-block-main-title">
                                Hello Guys!!
                            </div>
                            <div className="main-card-block-main-content">
                                Hoodie is so Cooool!
                            </div>
                            <div className="main-card-block-main-date">
                                2022-03-17
                            </div>
                        </div>
                        <div className="main-card-block-tag">
                            <div className="main-card-block-tag-id">
                                thewooowon
                            </div>
                            <div className="main-card-block-tag-like">
                                64
                            </div>
                        </div>
                    </div>
                    <div className="main-card-block sparkling">
                        <div className="main-card-block-image"></div>
                        <div className="main-card-block-main">
                            <div className="main-card-block-main-title">
                                Hello Guys!!
                            </div>
                            <div className="main-card-block-main-content">
                                Hoodie is so Cooool!
                            </div>
                            <div className="main-card-block-main-date">
                                2022-03-17
                            </div>
                        </div>
                        <div className="main-card-block-tag">
                            <div className="main-card-block-tag-id">
                                thewooowon
                            </div>
                            <div className="main-card-block-tag-like">
                                64
                            </div>
                        </div>
                    </div>
                    <div className="main-card-block sparkling">
                        <div className="main-card-block-image"></div>
                        <div className="main-card-block-main">
                            <div className="main-card-block-main-title">
                                Hello Guys!!
                            </div>
                            <div className="main-card-block-main-content">
                                Hoodie is so Cooool!
                            </div>
                            <div className="main-card-block-main-date">
                                2022-03-17
                            </div>
                        </div>
                        <div className="main-card-block-tag">
                            <div className="main-card-block-tag-id">
                                thewooowon
                            </div>
                            <div className="main-card-block-tag-like">
                                64
                            </div>
                        </div>
                    </div>
                </div>
                <div className="main-card">
                    <div className="main-card-block sparkling">
                        <div className="main-card-block-image"></div>
                        <div className="main-card-block-main">
                            <div className="main-card-block-main-title">
                                Hello Guys!!
                            </div>
                            <div className="main-card-block-main-content">
                                Hoodie is so Cooool!
                            </div>
                            <div className="main-card-block-main-date">
                                2022-03-17
                            </div>
                        </div>
                        <div className="main-card-block-tag">
                            <div className="main-card-block-tag-id">
                                thewooowon
                            </div>
                            <div className="main-card-block-tag-like">
                                64
                            </div>
                        </div>
                    </div>
                    <div className="main-card-block sparkling">
                        <div className="main-card-block-image"></div>
                        <div className="main-card-block-main">
                            <div className="main-card-block-main-title">
                                Hello Guys!!
                            </div>
                            <div className="main-card-block-main-content">
                                Hoodie is so Cooool!
                            </div>
                            <div className="main-card-block-main-date">
                                2022-03-17
                            </div>
                        </div>
                        <div className="main-card-block-tag">
                            <div className="main-card-block-tag-id">
                                thewooowon
                            </div>
                            <div className="main-card-block-tag-like">
                                64
                            </div>
                        </div>
                    </div>
                    <div className="main-card-block sparkling">
                        <div className="main-card-block-image"></div>
                        <div className="main-card-block-main">
                            <div className="main-card-block-main-title">
                                Hello Guys!!
                            </div>
                            <div className="main-card-block-main-content">
                                Hoodie is so Cooool!
                            </div>
                            <div className="main-card-block-main-date">
                                2022-03-17
                            </div>
                        </div>
                        <div className="main-card-block-tag">
                            <div className="main-card-block-tag-id">
                                thewooowon
                            </div>
                            <div className="main-card-block-tag-like">
                                64
                            </div>
                        </div>
                    </div>
                    <div className="main-card-block sparkling">
                        <div className="main-card-block-image"></div>
                        <div className="main-card-block-main">
                            <div className="main-card-block-main-title">
                                Hello Guys!!
                            </div>
                            <div className="main-card-block-main-content">
                                Hoodie is so Cooool!
                            </div>
                            <div className="main-card-block-main-date">
                                2022-03-17
                            </div>
                        </div>
                        <div className="main-card-block-tag">
                            <div className="main-card-block-tag-id">
                                thewooowon
                            </div>
                            <div className="main-card-block-tag-like">
                                64
                            </div>
                        </div>
                    </div>
                </div>
                <div className="main-card">
                    <div className="main-card-block sparkling">
                        <div className="main-card-block-image"></div>
                        <div className="main-card-block-main">
                            <div className="main-card-block-main-title">
                                Hello Guys!!
                            </div>
                            <div className="main-card-block-main-content">
                                Hoodie is so Cooool!
                            </div>
                            <div className="main-card-block-main-date">
                                2022-03-17
                            </div>
                        </div>
                        <div className="main-card-block-tag">
                            <div className="main-card-block-tag-id">
                                thewooowon
                            </div>
                            <div className="main-card-block-tag-like">
                                64
                            </div>
                        </div>
                    </div>
                    <div className="main-card-block sparkling">
                        <div className="main-card-block-image"></div>
                        <div className="main-card-block-main">
                            <div className="main-card-block-main-title">
                                Hello Guys!!
                            </div>
                            <div className="main-card-block-main-content">
                                Hoodie is so Cooool!
                            </div>
                            <div className="main-card-block-main-date">
                                2022-03-17
                            </div>
                        </div>
                        <div className="main-card-block-tag">
                            <div className="main-card-block-tag-id">
                                thewooowon
                            </div>
                            <div className="main-card-block-tag-like">
                                64
                            </div>
                        </div>
                    </div>
                    <div className="main-card-block sparkling">
                        <div className="main-card-block-image"></div>
                        <div className="main-card-block-main">
                            <div className="main-card-block-main-title">
                                Hello Guys!!
                            </div>
                            <div className="main-card-block-main-content">
                                Hoodie is so Cooool!
                            </div>
                            <div className="main-card-block-main-date">
                                2022-03-17
                            </div>
                        </div>
                        <div className="main-card-block-tag">
                            <div className="main-card-block-tag-id">
                                thewooowon
                            </div>
                            <div className="main-card-block-tag-like">
                                64
                            </div>
                        </div>
                    </div>
                    <div className="main-card-block sparkling">
                        <div className="main-card-block-image"></div>
                        <div className="main-card-block-main">
                            <div className="main-card-block-main-title">
                                Hello Guys!!
                            </div>
                            <div className="main-card-block-main-content">
                                Hoodie is so Cooool!
                            </div>
                            <div className="main-card-block-main-date">
                                2022-03-17
                            </div>
                        </div>
                        <div className="main-card-block-tag">
                            <div className="main-card-block-tag-id">
                                thewooowon
                            </div>
                            <div className="main-card-block-tag-like">
                                64
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default Main;